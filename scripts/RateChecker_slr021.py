...
def load_prsc_in_RAM(prsc_arr, SLR, sel):
    if SLR == 0:
        if sel == 0:
            hw.getNode("payload.SLR0_monitor.prescale_factor").writeBlock(prsc_arr)
        elif sel == 1:
            hw.getNode("payload.SLR0_monitor.prescale_factor_prvw").writeBlock(prsc_arr)
        else:
        ...
        raise Exception("Available SLRs are 0 and 2")
    hw.dispatch()


def request_update_prescale():
    hw.getNode("payload.SLR0_monitor.CSR.ctrl.request_pulse_update").write(0)
    hw.getNode("payload.SLR2_monitor.CSR.ctrl.request_pulse_update").write(0)
    hw.getNode("payload.SLR0_monitor.CSR.ctrl.request_pulse_update").write(1)
...
def load_mask_arr(mask_arr, SLR):
    if SLR == 0:
        hw.getNode("payload.SLR0_monitor.trgg_mask").writeBlock(mask_arr)
    elif SLR == 2:
...
def load_BXmask_arr(BXmask_arr, SLR):
    if SLR == 0:
        hw.getNode("payload.SLR0_monitor.algo_bx_masks.data_0_31").writeBlock(BXmask_arr[0])
        hw.getNode("payload.SLR0_monitor.algo_bx_masks.data_32_63").writeBlock(BXmask_arr[1])
        hw.getNode("payload.SLR0_monitor.algo_bx_masks.data_64_95").writeBlock(BXmask_arr[2])
        hw.getNode("payload.SLR0_monitor.algo_bx_masks.data_96_127").writeBlock(BXmask_arr[3])
        hw.getNode("payload.SLR0_monitor.algo_bx_masks.data_128_159").writeBlock(BXmask_arr[4])
        hw.getNode("payload.SLR0_monitor.algo_bx_masks.data_160_191").writeBlock(BXmask_arr[5])
        hw.getNode("payload.SLR0_monitor.algo_bx_masks.data_192_223").writeBlock(BXmask_arr[6])
        hw.getNode("payload.SLR0_monitor.algo_bx_masks.data_224_255").writeBlock(BXmask_arr[7])
        hw.getNode("payload.SLR0_monitor.algo_bx_masks.data_256_287").writeBlock(BXmask_arr[8])
        hw.getNode("payload.SLR0_monitor.algo_bx_masks.data_288_319").writeBlock(BXmask_arr[9])
        hw.getNode("payload.SLR0_monitor.algo_bx_masks.data_320_351").writeBlock(BXmask_arr[10])
        hw.getNode("payload.SLR0_monitor.algo_bx_masks.data_352_383").writeBlock(BXmask_arr[11])
        hw.getNode("payload.SLR0_monitor.algo_bx_masks.data_384_415").writeBlock(BXmask_arr[12])
        hw.getNode("payload.SLR0_monitor.algo_bx_masks.data_416_447").writeBlock(BXmask_arr[13])
        hw.getNode("payload.SLR0_monitor.algo_bx_masks.data_448_479").writeBlock(BXmask_arr[14])
        hw.getNode("payload.SLR0_monitor.algo_bx_masks.data_480_511").writeBlock(BXmask_arr[15])
        hw.getNode("payload.SLR0_monitor.algo_bx_masks.data_512_543").writeBlock(BXmask_arr[16])
        hw.getNode("payload.SLR0_monitor.algo_bx_masks.data_544_575").writeBlock(BXmask_arr[17])
    elif SLR == 2:
...
def read_cnt_arr(SLR, sel):
    if SLR == 0:
        if sel == 0:
            cnt = hw.getNode("payload.SLR0_monitor.cnt_rate_before_prsc").readBlock(576)
        elif sel == 1:
            cnt = hw.getNode("payload.SLR0_monitor.cnt_rate_after_prsc").readBlock(576)
        elif sel == 2:
            cnt = hw.getNode("payload.SLR0_monitor.cnt_rate_after_prsc_prvw").readBlock(576)
        elif sel == 3:
            cnt = hw.getNode("payload.SLR0_monitor.cnt_rate_pdt").readBlock(576)
        else:
...
def read_trigg_cnt(sel):
    if sel == 0:
        cnt = hw.getNode("payload.SLR1_FINOR.cnt_rate_finor").readBlock(8)
    elif sel == 1:
        cnt = hw.getNode("payload.SLR1_FINOR.cnt_rate_finor_pdt").readBlock(8)
    else:
        raise Exception("Selector is not in [0,1]")
    hw.dispatch()
    return np.array(cnt, dtype=np.uint32)
...
# Set the bxmasks
bxmask_0 = (2**32-1)*np.ones((18,4096), dtype=np.uint32)
bxmask_2 = (2**32-1)*np.ones((18,4096), dtype=np.uint32)

# Set the masks
masks_0 = np.zeros(144, dtype=np.uint32)
masks_0[0:144] = (2 ** 32-1) * np.ones(1, dtype=np.uint32)

masks_2 = np.zeros(144, dtype=np.uint32)
masks_2[0:144] = (2 ** 32 - 1) * np.ones(1, dtype=np.uint32)

prsc_fct_0      = np.uint32(100 * np.ones(576))  # 1.00
prsc_fct_2      = np.uint32(100 * np.ones(576))  # 1.00
prsc_fct_prvw_0 = np.uint32(100 * np.ones(576))  # 1.00
...
if args.test == "random":
	prsc_fct_0[np.int16(index_high-576)]  = np.uint32(np.random.randint(100,2**24,len(index_high)))
	prsc_fct_2[np.int16(index_low)]       = np.uint32(np.random.randint(100,2**24,len(index_low)))
elif args.test == "linear":
	prsc_fct_0[np.int16(index_high-576)]  = np.uint32(np.linspace(100,2**24-1,len(index_high)))
	prsc_fct_2[np.int16(index_low)]       = np.uint32(np.linspace(100,2**24-1,len(index_low)))

prsc_fct = np.vstack((prsc_fct_2,prsc_fct_0)).flatten()
load_prsc_in_RAM(prsc_fct_0, 0, 0)
load_prsc_in_RAM(prsc_fct_2, 2, 0)

if args.test == "random":
	prsc_fct_prvw_0[np.int16(index_high-576)]  = np.uint32(np.random.randint(100,2**24, len(index_high)))
	prsc_fct_prvw_2[np.int16(index_low)]       = np.uint32(np.random.randint(100,2**24, len(index_low)))
elif args.test == "linear":
	prsc_fct_prvw_0[np.int16(index_high-576)]  = np.uint32(np.linspace(100,2**24-1,len(index_high)))
	prsc_fct_prvw_2[np.int16(index_low)]       = np.uint32(np.linspace(100,2**24-1,len(index_low)))

prsc_fct_prvw = np.vstack((prsc_fct_prvw_2,prsc_fct_prvw_0)).flatten()
load_prsc_in_RAM(prsc_fct_prvw_0, 0, 1)
load_prsc_in_RAM(prsc_fct_prvw_2, 2, 1)

load_BXmask_arr(bxmask_0, 0)
load_BXmask_arr(bxmask_2, 2)

load_mask_arr(masks_0, 0)
load_mask_arr(masks_2, 2)


cnt_before_0 = read_cnt_arr(0, 0)
cnt_before_2 = read_cnt_arr(2, 0)
cnt_before   = np.vstack((cnt_before_2,cnt_before_0)).flatten()
cnt_after_0  = read_cnt_arr(0, 1)
cnt_after_2  = read_cnt_arr(2, 1)
cnt_after    = np.vstack((cnt_after_2,cnt_after_0)).flatten()
cnt_prvw_0   = read_cnt_arr(0, 2)
cnt_prvw_2   = read_cnt_arr(2, 2)
cnt_prvw     = np.vstack((cnt_prvw_2,cnt_prvw_0)).flatten()
cnt_pdt_0    = read_cnt_arr(0, 3)
cnt_pdt_2    = read_cnt_arr(2, 3)
cnt_pdt      = np.vstack((cnt_pdt_2,cnt_pdt_0)).flatten()
